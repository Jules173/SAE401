...
/*require './Repository/AdministrationBDD.php';
require './Repository/AttributionBDD.php';
require './Repository/BinBDD.php';
require './Repository/CompetenceBDD.php';
require './Repository/EtudiantBDD.php';
require './Repository/MoyenneCompetenceBDD.php';
require './Repository/MoyenneEleveBDD.php';
require './Repository/SemestreBDD.php';
require './Repository/UserBDD.php';
require './Repository/ValidationBDD.php';*/

require './Entity/Administration.php';
require './Entity/Attribution.php';
require './Entity/Bin.php';
require './Entity/Competence.php';
require './Entity/Etudiant.php';
require './Entity/MoyenneCompetence.php';
require './Entity/MoyenneEleve.php';
require './Entity/Semestre.php';
require './Entity/User.php';
require './Entity/Validation.php';
...
class DB {
    ...
    {
        $requete = 'SELECT idComp FROM competence WHERE code = ? AND idSemestre = ?;';
    ...
    public function insertEtudiant($idEtu, $codenip, $civ, $nom, $prenom, $grpTD, $grpTP, $bac, $specialite)
    {
        $requete = 'INSERT INTO Etudiant (idEtu, codenip, civ, nom, prenom, grpTD, grpTP, bac, specialite) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
        $tparam = array($idEtu, $codenip, $civ, $nom, $prenom, $grpTD, $grpTP, $bac, $specialite);
        return $this->execMaj($requete, $tparam);
    }
	...
	public function insertCompetencee($code, $idSemestre)
    {
        $requete = 'INSERT INTO Competence (code, idSemestre) 
                    VALUES (?, ?)';
        $tparam = array($code, $idSemestre);
        return $this->execMaj($requete, $tparam);
    }
    ...
    public function ifExist($valeur, $table, $champ, $pdo)
    {
		...
		$resultats = $this->execQuery($requete, [$valeur], null);
	...
	public function getCompetencesModulesBySemestre($idSemestre) {
		// Requête SQL pour récupérer les compétences et les modules associés à un semestre spécifique
		$requete = 'SELECT c.code AS code_comp, b.codeBin AS code_Bin
					FROM Competence c
					JOIN Attribution a ON c.idComp = a.idComp
					JOIN Bin b ON a.idBin = b.idBin
					WHERE c.idSemestre = ?';
		
		// Exécution de la requête avec le paramètre $idSemestre
		$resultats = $this->execQuery($requete, [$idSemestre], null);
	
		// Retourner les résultats
		return $resultats;
	}

	public function getInfosEtudiant($idEtu) {
		$requete = 'SELECT e.nom, e.prenom, m.moyenne AS moyenne_bin, mc.bonus AS moyenne_comp
					FROM Etudiant e
					LEFT JOIN MoyenneEleve m ON e.idEtu = m.idEtu
					LEFT JOIN MoyenneCompetence mc ON e.idEtu = mc.idEtu
					WHERE e.idEtu = ?';
		$resultats = $this->execQuery($requete, [$idEtu], null);
		return $resultats;
	}
